...
%\todo{See how this explains fiscal sociology (1): Schneider:2012rr}
%	- Fiscal sociology is divided in capacity and consent/legitimacy (p. 35). I do the first one (second one is Mahon and `fiscal contract'). Martin2009a calls the first one `coercion and conflict.' 
%		- I could use Tilly's definition of `state capacity' in Martin2009a (he wrote the last chapter). 
%	- ``this tradition places war at the heart of greater tax efforts'' (p. 35). Here I could say that wars have been absent (Centeno). 

%\todo{See how this explains fiscal sociology (2) Martin2009a}


%Introducing Fiscal Sociology // connection with state capacities.
According to the fiscal sociology approach, the great modern cleavage was not the rise of capitalism (Marx) nor the rise of modern bureaucracy (Weber), but the rise of the ``tax state,'' which developed institutions to penetrate individual economies.\footnote{\citet[298]{Moore2004a}. This view is also shared by \citet[100]{Schumpeter:1991yq} and \citet[42]{Lewis:1965nr} - See epigraphs.} Given this high degree of state penetration, taxation offers the key for a theory of the state.\footnote{\citet[99]{Musgrave1992}.} In fact, following Tilly's definition of \emph{state capacities}\footnote{``The \emph{extent to which interventions of state agents in existing non-state resources, activities, and interpersonal connections alter existing distributions of those resources, activities, and interpersonal connections as well as relations among those distributions}.'' In \citet[173]{Martin2009a} (emphases in the original).}

talk about obligation: Martin2009a first chapter

% conflictual nature
The fiscal sociology paradigm is vast, and without trying to survey all of it, this paper follows Schumpeter's conception in that it sees ``taxation in terms of group conflicts [and] class interests.''\footnote{\citet[14]{Monson2015}.} For example, Goldscheid (1925), in \citet[168]{Campbell1993}, argues that ``tax struggles are among the oldest forms of class struggle.''




Borrowing from this literature, this paper argues that the origins of the fiscal apparatus are rooted in sectoral and class conflicts. 
...
\todo{Add to the section when I explain that there were two elites}
