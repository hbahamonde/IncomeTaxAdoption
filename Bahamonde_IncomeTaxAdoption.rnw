...
Using the estimations from Model 1 in \autoref{results:1}, I follow \citet{Gandrud2015} and \citet{King2000}, and in \autoref{fig:simulation:plots} simulate \Sexpr{nsim} times the \Sexpr{qi} of implementing the income tax law conditional on industrial and agricultural growth rates.\footnote{\citet[15]{Box-Steffensmeier2004} explain that the \Sexpr{qi} is the most common quantity of interest analysts focus on.} I do not combine both variables since \todo{Find citation: relative to zero.}. Even though the outcome of interest does \emph{not} depend \emph{directly} on time,\footnote{Please refer to \autoref{fig:survival:plot}.} countries \emph{in time} {\bf do} experience an increase in their sectoral outputs.\footnote{Please refer to \autoref{fig:incometax}.} Consequently, it will be necessary to model the natural tendency of sectoral outputs to grow in time by allowing each variable to vary with time.\footnote{The economics literature refers to these kinds of time series `integrated' or I(1) processes.} Since the \Sexpr{qi} ``is the probability that a case will fail at time $t$''\footnote{\citet[231]{Licht2011}.}, the \Sexpr{qi} allows a dependency on both time and the covariates.\footnote{\citet[15]{Box-Steffensmeier2004}.}

\todo{comment results of simulation here.}
